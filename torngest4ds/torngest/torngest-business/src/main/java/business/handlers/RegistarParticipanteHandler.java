	...
	private void gerarConfrontosEquipa(TorneioEquipa torE) throws TornGesException {
		...
		for(int i = 0; i < listaEquipas.size(); i++) {
			...
			for(int j = (i+1); j < listaEquipas.size(); j++) {
				...
				l1 = l1.stream().sorted(Comparator.comparing(Jogador::getPontos)).collect(Collectors.toList());
				l2 = l2.stream().sorted(Comparator.comparing(Jogador::getPontos)).collect(Collectors.toList());
